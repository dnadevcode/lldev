classdef (Sealed) AppSession < handle
    ...
    properties (Constant, Hidden)
        AppSessionsDataPoolID = sprintf('%sAppSessions', AppMgr.AppDataPoolViewScreen.HideDataPoolPrefix);
    end
    ...
    methods (Static)
        function appSess = get_instance(varargin)
            ...
            if isempty(persistentLocalAppSess) || not(isvalid(persistentLocalAppSess))
                ...
                [~, idx] = FancyGUI.FancyInput.dropdown_dialog('App Session Selection', 'Choose a session to use', relatedSessionIDs);
