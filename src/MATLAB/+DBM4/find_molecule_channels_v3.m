function [peaksToPlot, peakcc,peakint] = find_molecule_channels_v3(image, sets)
...
%    rowMeans = nanmean(image.*mask,2);
   ...
   % in case non-uniform illumination or sth like that:
    ...
    import DBM4.find_channels_based_on_intensity_max;
    corrs = find_channels_based_on_intensity_max(image1,image2,sets.numPts);
%     corrs2 = find_channels_based_on_corr(image,2);
    
       
    import DBM4.find_channels_based_on_intensity_max;
    corrsFarAway = find_channels_based_on_intensity_max(image1,circshift(image2,[0,50]),sets.numPts);
