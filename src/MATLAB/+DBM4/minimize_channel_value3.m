function [value] = minimize_channel_value3(image, par1, par2)
...
%         A = image(:,i:i+round(par2)-1);
        interpValue=i-floor(i)+1;
%         subImage{idx} = A(:,width:end-width+1);
        subImage{idx} = interp2(X,Y,image(:,floor(i):floor(i)+1),repmat(interpValue,1,size(image,1)),1:size(image,2));
    end
