function [scoresAtBestN, xcorrAtBestN, coverageLenAtBestN,...
        flipAtBestN, circShiftAtBestN, offsetAtBestN,...
        scores, xcorrs, coverageLens,...
        firstOffset, maxPossibleCoverageLen] = get_top_scores(...
        seqA, seqB, bitmaskA, bitmaskB, circularA, circularB,...
        allowExtraCropping, topN, fn_compute_scores)
    % get_top_scores
    %  Computes and returns the scores, Pearson cross-correlation
    %   coefficents, and coverage lengths that can be found for
    %   pairs of sequence-bitmask pair after reorienting them in
    %   different ways as allowed for based on whether they are
    %    circular orlinear
    %
    % Inputs:
    ...
    import SignalRegistration.XcorrAlign.get_circ_circ_xcorrs;
    ...
    import SignalRegistration.XcorrAlign.get_lin_lin_xcorrs;
    import SignalRegistration.XcorrAlign.get_max_possible_coverage_len;
    import SignalRegistration.XcorrAlign.extract_n_best;
    ...
    if (circularA)
        if (circularB)
            [xcorrs, coverageLens, firstOffset] = get_circ_circ_xcorrs(seqB, seqA, bitmaskB, bitmaskA);
        else % b is linear
            [scoresAtBestN, xcorrAtBestN, coverageLenAtBestN, flipAtBestN, circShiftAtBestN, offsetAtBestN, scores, xcorrs, coverageLens, firstOffset, maxPossibleCoverageLen] = get_top_scores(seqB, seqA, bitmaskB, bitmaskA, circularB, circularA, allowExtraCropping, topN);
            return;
        end
    else % a is linear
        if (circularB)
            if (lenB < lenA)
                % warning('Circular sequence is shorter than linear sequence');
            end
            if (allowExtraCropping)
                [xcorrs, coverageLens, firstOffset] = get_lin_circ_xcorrs(seqA, seqB, bitmaskA, bitmaskB);
            else
                [xcorrs, coverageLens, firstOffset] = get_no_extra_cropping_lin_circ_xcorrs(seqA, seqB, bitmaskA, bitmaskB);
            end
        else % b is linear
            [xcorrs, coverageLens, firstOffset] = get_lin_lin_xcorrs(seqA, seqB, bitmaskA, bitmaskB);
        end
    end
    maxPossibleCoverageLen = get_max_possible_coverage_len(bitmaskA, bitmaskB, circularA, circularB);

    scores = fn_compute_scores(xcorrs, coverageLens, maxPossibleCoverageLen);
    [scoresAtBestN, xcorrAtBestN, coverageLenAtBestN, flipAtBestN, circShiftAtBestN, offsetAtBestN] = extract_n_best(scores, xcorrs, coverageLens, firstOffset, topN);
end
