function [ xcorrs, numElts ] = masked_pcc_corr( shortVec,longVec,w1,w2 )
    ...
    ccF = xiyi./denominator - numForward.*xbar.*ybar./denominator;
    ...
    numReverse =  (ifft(w2fft.*conVec));
    % ybar
    ybar =  (ifft(fft(longVec).*conVec))./numReverse;
    ySq =  (ifft(fft(longVec.^2).*conVec));

    % main part. Compute xiyi
    conVec = conj(fft(shortVecFlip,longLength));
    xiyi = (ifft(fft(longVec).*conVec));
    xbar =  (ifft(w2fft.*conVec))./numReverse;

    % now, compute mean of the first vector sq
    conVec = conj(fft(shortVecFlip.^2,longLength));
    xSq =  (ifft(w2fft.*conVec));

    denominator = sqrt(xSq-numReverse.*xbar.^2).*sqrt(ySq-numReverse.*ybar.^2);
    %numerator = xiyi-numForward.*xbar.*ybar;
    % now, compute mean of the second vector sq
    %ccB = numerator./denominator;
	ccB = xiyi./denominator - numReverse.*xbar.*ybar./denominator;

    xcorrs = [ccF;ccB];
    
    numElts =[numForward; numReverse]; % number of non-nan elements. For this calculate \sum w_iw_j
