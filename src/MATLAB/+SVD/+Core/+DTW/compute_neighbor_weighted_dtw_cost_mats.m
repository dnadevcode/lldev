function [costMat, accumCostMat] = compute_neighbor_weighted_dtw_cost_mats(seq1, seq2, dist)
    %Computes the neighbour-weighted cost matrix and accumulated
    %cost matrix for two barcodes. Modifies classic DTW 
    ...
    for idxA = 1:lenA
        for idxB = 1:lenB
            ...
            currCost = sum(abs(seqA(rangeIdxsA) - seqB(rangeIdxsB)));
