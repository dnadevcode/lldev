function [ ] = plot_contigs3(refCurve, contigItems,titleName, ccValueMatrix, pValueMatrix,colors )
    ...
    if ~isempty(pValueMatrix)
       ...
       % h1{1} = plot(refCurve, 'black'); % or whatever is appropriate
        for contigNum=1:size(contigItems,2) % does not include flip option
            if	contigItems.isRemoved(contigNum) == 0
                if contigItems.isReversed(contigNum) == 0
                    pVal = pValueMatrix(contigNum,round(contigItems.PredictedPlacePxStart(contigNum))+length(refCurve));
                else
                    pVal = pValueMatrix(contigNum,round(contigItems.PredictedPlacePxStart(contigNum)));
                end
                %pVal
                %contigNum
               % contigItems{contigNum}.PredictedPlacePxStart
                ...
                startBarcode = round(contigItems.PredictedPlacePxStart(contigNum))+bit-0;
                stopBarcode = round(contigItems.PredictedPlacePxEnd(contigNum));
                contigNum


                barc = contigItems.barcode{contigNum};
                ...
                a = pVal * 10^(-b);
                a*10^b
                legendInfo{nInd+1} = [ '$\hat C_{'  contigItems.name{contigNum} '}=$' num2str(max(ccValueMatrix(contigNum,:)),'%.1f ') ', p-val $ =' num2str(a,'%.2f ') '\cdot 10 ^{' num2str(b) ' }$']; % or whatever is appropriate
                nInd = nInd+1;
            end
            %size(legendInfo)
...
% 
    ...
    for contigNum=1:size(contigItems,2) % does not include flip option
    	if contigItems.BelongsToSeq(contigNum) == 1
             startBarcode = round(contigItems.corPlacePxStart(contigNum));
             stopBarcode = round(contigItems.corPlacePxEnd(contigNum));
             if stopBarcode < startBarcode
                 pixelList = [startBarcode:length(refCurve)];
                 xx2= [1:stopBarcode]; 
        ...
        ylim([-4.2 3])
  plot([20+0 20+round(34000/592)],[-4 -4],'linewidth',2,'color','black') % some hardcorded numbers that we should have
    text(20+0,-3.8,'$l_{\rm thresh}$','Interpreter','latex')
% 
        subplot(1,4,4);
        clear legendInfo;
