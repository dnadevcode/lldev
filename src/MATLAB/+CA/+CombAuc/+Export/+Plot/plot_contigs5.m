function [ ] = plot_contigs5(refCurve, contigItems,titleName, ccValueMatrix, pValueMatrix,colors )
        ...
        for contigNum=1:size(contigItems,2) % does not include flip option
            if	contigItems{contigNum}.isRemoved == 0 && contigItems{contigNum}.isTooShort == 0 ;
               ...
               % contigItems{contigNum}.PredictedPlacePxStart
                ...
                a = pVal * 10^(-b);
                a*10^b
                legendInfo{nInd+1} = [ '$\hat C_{'  contigItems{contigNum}.name '}=$' num2str(max(ccValueMatrix(contigNum,:)),'%.2f ') ', p-val $ =' num2str(a,'%.1f ') '\cdot 10 ^{' num2str(b) ' }$']; % or whatever is appropriate
                        nInd = nInd+1;
            end
            %size(legendInfo)
            %legend([h1{1} h1{2} ] ,legendInfo)
            legend(legendInfo,'Interpreter','latex','location', 'se');
% 
    %
        end    
    ...
    for contigNum=1:size(contigItems,2) % does not include flip option
    	if contigItems{contigNum}.BelongsToSeq == 1
             ...
             if stopBarcode < startBarcode
                ...
                % text(u(round(end/4)),max(refCurve)+0.2, strcat([ num2str(contigItems{contigNum}.name)]),'Interpreter','latex')
