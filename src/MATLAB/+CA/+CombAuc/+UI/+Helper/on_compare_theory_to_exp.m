function [ comparisonStructure ] = on_compare_theory_to_exp( rawBarcodes,rawBitmasks,theorBar,theorBit,sets,str)
    ...
    import CA.CombAuc.Core.Comparison.get_cc_fft;
    ...
    for i=1:length(rawBarcodes)
        ...
        parfor j=1:length(stretchFactors)
            ...
            if length(barC) > length(theorBar.Value)
                 [xcorrs,~,~] =  CA.CombAuc.Core.Comparison.get_cc_fft(theorBar.Value,barC, theorBit.Value,barB);
               % [xcorrs, ~, ~] = get_no_crop_lin_circ_xcorrs(theorBar.Value,barC, theorBit.Value,barB);
            else
                [xcorrs, ~, ~] = CA.CombAuc.Core.Comparison.get_cc_fft(barC, theorBar.Value, barB,theorBit.Value);
            ...
            [rezMax{j}.maxcoef,rezMax{j}.pos,rezMax{j}.or] = CA.CombAuc.UI.Helper.get_best_parameters(xcorrs,length(barC) );
        end       
