function [hcaSessionStruct] = compare_theory_to_exp(hcaSessionStruct, sets )
    ...
    import CA.CombAuc.UI.Helper.on_compare_theory_to_exp
    hcaSessionStruct.comparisonStructure = on_compare_theory_to_exp(rawBarcodes,rawBitmasks, hcaSessionStruct.theorySeq,hcaSessionStruct.bitmask,sets);
    ...
    if sets.filterSettings.filter==1
        ...
        if sets.filterSettings.filterMethod == 0 
            hcaSessionStruct.comparisonStructureFiltered = on_compare_theory_to_exp(rawBarcodes,rawBitmasks, hcaSessionStruct.theorySeq,hcaSessionStruct.bitmask,sets,1);   
        else  
            hcaSessionStruct.comparisonStructureFiltered = on_compare_theory_to_exp(rawBarcodes,rawBitmasks, hcaSessionStruct.theorySeq,hcaSessionStruct.bitmask,sets);   
        end
