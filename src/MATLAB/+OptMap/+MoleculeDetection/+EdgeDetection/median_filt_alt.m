...
%       mat - output thresholded image
    ...
    [dat,vals] = hist(fullmat,sqrt(length(fullmat)));
    [pos,pk] = findpeaks(imgaussfilt(dat,10),'NPeaks',1);
    leftHalfDist = fullmat(fullmat<vals(pk));
    
    meanD = mean(leftHalfDist)+std(leftHalfDist)*sqrt(2/pi);
    varD = std(leftHalfDist)^2*(1-2/pi);
    
    bg = fullmat(fullmat<meanD+5*sqrt(varD));
