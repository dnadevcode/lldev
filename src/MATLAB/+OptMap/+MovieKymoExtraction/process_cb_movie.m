function [movieProcessingResultsStruct] = process_cb_movie(srcTiffFilepath, settings)
    %#ok<*ASGLU>
    %#ok<*STRNU>
    ...
    fprintf('Importing movie...\n');
    fprintf(' (''%s'')\n', settings.import.srcTiffFilepath);
    ...
    if not(settings.cleanup.skipCleanup)
        ...
        if not(settings.cleanup.skipCleanupReport)
            if height(details.cleanup.outliers.table) > 0
                ...
                fprintf(' Outliers/Total: %d/%d\n', height(details.cleanup.outliers.table), numel(tmp_movie.cleaned));
    ...
    %--------- DETECT CHANNELS
	import OptMap.MovieKymoExtraction.get_channel_indices;
    disp('Detecting occupied movie nanochannels...');
    
    tmp_channelProfileSignal = median(max(permute(data.movie.processed, [3, 2, 1]), [], 3));
    details.channelDetection.channelProfileSignal = tmp_channelProfileSignal;
    tmp_minValDistBetweenAdjLocalExtrema = settings.channelDetection.minAdjLocalExtremaDistRelToRange*(max(tmp_channelProfileSignal) - min(tmp_channelProfileSignal));
    [details.channelDetection.channelLabeling] = get_channel_indices(details.channelDetection.channelProfileSignal, settings.channelDetection.channelWidthPx, settings.channelDetection.channelGapWidthPx, settings.channelDetection.maxSigmaNonBlip, tmp_minValDistBetweenAdjLocalExtrema);
