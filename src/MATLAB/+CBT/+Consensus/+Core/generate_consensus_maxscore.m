function [ consensusStruct ] = generate_consensus_maxscore( barcodes, bitmasks, sets )
    ...
    % code by Albertas Dvirnas, edited 03/05/21
...
%     end
    ...
    for j=1:numBarcodes
        rawBar(j,1:lengths(j)) = barcodes{j}; %barcodeNormalizationFunction(barcodes{j},bgMeanApprox{j});
    ...
    maxscore = zeros(numBarcodes-1,numBarcodes-1);
   ...
   % G = digraph();
    ...
    for barcodeIdxA = 1:numBarcodes-1
        ...
        for barcodeIdxB = barcodeIdxA+1:numBarcodes
            ...
            [xcorrs,numElts] = masked_pcc_corr(barcodeA,barcodeB,bitmaskA,bitmaskB);
    ...
    % something
     ...
     consensusStruct.treeStruct.maxScore = zeros(1,numBarcodes-1);
...
%          max(maxcoef(:))
         ...
         [consensusStruct.treeStruct.maxScore(bb), I] = max(maxscore(:));
%          [consensusStruct.treeStruct.maxScore(bb), I] = max(maxcoef(:));
%         [I_row, I_col] = ind2sub(size(maxcoef),I);
%         1-consensusStruct.treeStruct.maxScore(bb)/sqrt(411)
        
        
         % 
         consensusStruct.treeStruct.scoreMat{bb} = maxscore;
        consensusStruct.treeStruct.barToAverage{bb} = barToAverage;
        
        [I_row, I_col] = ind2sub(size(maxscore),I);
        consensusStruct.treeStruct.maxCorCoef(bb)= maxcoef(I_row,I_col);
        ...
        [~,stPos] = arrayfun(@(x) find(positions(x,:)==1),1:size(positions,1)); % find start and end position
%         [~,stOr] = arrayfun(@(x) find(positions(x,:)==2),1:size(positions,1));
        [~,stOr] = arrayfun(@(x) find(positions(x,:)==2),1:size(positions,1));
        barOr = (stPos > stOr) + 1; % bugcheck: should look at this on a circular array instead
        idxsBugCheck = find(abs(stPos-stOr) > 1);
        barOr(idxsBugCheck) = ((stPos(idxsBugCheck)+size(positions,2)-stOr(idxsBugCheck))==1)+1;
        ...
        for barcodeIdxA = 1:rowToKeep-1
            ...
            [ b, ix ] = sort( f(:), 'descend' );
            indx = b(1) ;
            maxscore(barcodeIdxA,rowToKeep-1) = indx;
            or(barcodeIdxA,rowToKeep-1) =s(ix(1));
            pos(barcodeIdxA,rowToKeep-1) = ix(1);
            maxcoef(barcodeIdxA,rowToKeep-1) =  xcorrs(s(ix(1)),ix(1));
        end
        ...
        for barcodeIdxA = rowToKeep+1:numBarcodes-bb+1
            ...
            [ b, ix ] = sort( f(:), 'descend' );
            indx = b(1) ;
            maxscore(rowToKeep,barcodeIdxA-1) = indx;
            or(rowToKeep,barcodeIdxA-1) =s(ix(1));
            pos(rowToKeep,barcodeIdxA-1) = ix(1);
            maxcoef(rowToKeep,barcodeIdxA-1) =  xcorrs(s(ix(1)),ix(1));
        ...
        if rowToRemove <= size(maxscore,1)
            maxcoef(rowToRemove,:) = [];
            maxscore(rowToRemove,:) = [];
