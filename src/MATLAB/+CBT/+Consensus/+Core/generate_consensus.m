function [ consensusStruct ] = generate_consensus( barcodes, bitmasks, sets )
   ...
   % G = digraph();
    ...
    for barcodeIdxA = 1:numBarcodes-1
        ...
        for barcodeIdxB = barcodeIdxA+1:numBarcodes
            ...
            maxscore(barcodeIdxA,barcodeIdxB-1) = scores(s(ix(1)),ix(1));
...
%          max(maxcoef(:))
        ...
        consensusStruct.treeStruct.maxScore(bb)= maxscore(I_row,I_col);
...
%         [~,stOr] = arrayfun(@(x) find(positions(x,:)==2),1:size(positions,1));
    ...
    %         stOr  = [400  1]; % or [2 1] 
        ...
        for barcodeIdxA = 1:rowToKeep-1
            ...
            maxscore(barcodeIdxA,rowToKeep-1) =  scores(s(ix(1)),ix(1));
        end
        ...
        for barcodeIdxA = rowToKeep+1:numBarcodes-bb+1
            ...
            maxscore(I_row,barcodeIdxA-1) =  scores(s(ix(1)),ix(1));
