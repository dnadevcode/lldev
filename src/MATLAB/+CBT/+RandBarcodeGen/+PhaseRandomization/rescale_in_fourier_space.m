function [ outputData ] = rescale_in_fourier_space( inputData,newMean, newVar )
    %rescale_in_fourier_space
    % input
    %  inputData,newMean, newVar
...
%     (len2*a-newB(1).^2)
   % (len2*b-newB(1).^2)
    ...
    outputData = abs([newB(1) newB(2:end).*konst]);
  %  interpolatedData
