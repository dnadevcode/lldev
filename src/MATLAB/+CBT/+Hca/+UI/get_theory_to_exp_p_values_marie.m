function [hcaSessionStruct] = get_theory_to_exp_p_values_marie(hcaSessionStruct, sets )
   ...
   % import CA.CombAuc.Core.Comparison.cc_fft;
    ...
    if sets.prestretchMethod == 0
      ...
      %  sortedData =sort(xx(:));
        ...
        pValueMatrix = length(xx)*CA.CombAuc.Core.Comparison.compute_p_value(coefs,evdPar,'cc')
    %     
...
%         hcaSessionStruct.pValueResultsOneBarcode.rsq = rSquaredExact;
        ...
        if sets.filterSettings.filter==1
            ...
            pValueMatrixFiltered =  length(xx)*CA.CombAuc.Core.Comparison.compute_p_value(coefs,evdPar,'cc'); 
