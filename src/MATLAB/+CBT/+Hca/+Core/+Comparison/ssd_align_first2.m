function [ alignedKym,unAlignedKymoMask,alignedKymoMoleculeMask,backgroundKym, ssdCoef] = ssd_align_first2(kymoToAlign,barcodeConsensusSettings,edgeDetectionSettings,sets)
...
% %    
   ...
   unAlignedKymoMask= zeros(size(kymoToAlign));
   kym = cell(1,size(kymoToAlign,1));
   for i=1:size(kymoToAlign,1)
       
        kym{i} = imgaussfilt(kymoToAlign(i,:),stretchPar*filterSize);
        [idx1,~] = kmeans(kym{i}',numLev);
...
% %    
    unAlignedKymoMask = unAlignedKymoMoleculeMask;
    
    lengths = rightEdgeIdxs - leftEdgeIdxs;
    stdDev = std(rightEdgeIdxs - leftEdgeIdxs);
    avg = round(mean(rightEdgeIdxs - leftEdgeIdxs));

    oldW = size(kymoToAlign,2);
    newW = ceil(oldW*avg/min(lengths));

    stretchedKym = zeros(size(kymoToAlign,1),newW)+kymoToAlign(1,10);
    for i=1:size(kymoToAlign,1)
        stretchedRow = interp1(kymoToAlign(i,:),linspace(1,oldW,round(oldW*avg/lengths(i))));
        stretchedKym(i,1:length(stretchedRow)) = stretchedRow;
    end
    
    kymoToAlign = stretchedKym;
    unAlignedKymoMoleculeMask = zeros(size(kymoToAlign));
    kym = cell(1,size(kymoToAlign,1));
    for i=1:size(kymoToAlign,1)
