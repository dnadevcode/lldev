function [pVal] = get_pvalue_method_in(coefs, lengths, lenLong, filtPar, stretchFactors, nI )
...
%     lenLong = length(longTheory(:)); 
    ...
    for j=1:length(coefs)
            ...
            for k =1:nI
            	pV = @(x)  tLen*(1-normcdf(x,evT{1}{k}.mu, evT{1}{k}.sigma));
                if coefs(j) < xzeros{k}
                    pVl(k,j) = 1;
                else
                	pVl(k,j) = pV(coefs(j));
