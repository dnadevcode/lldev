function [] = exp_vs_exp_bitmasked_ui(...
        ts, ...
    ...
    % do the comparison
    import CBT.ExpComparison.Core.compare_exps_vs_exps_bitmasked;
    [a] = compare_exps_vs_exps_bitmasked(...
            stretchedConsensusBarcodes, ...
            stretchedConsensusBitmasks, ...
            eteSettings);

    import CBT.ExpComparison.Export.create_ete_session_structure;
   % import CBT.ExpComparison.Export.create_ete_session_struct;
    eteSessionStruct = create_ete_session_structure(comparisonResults,consensusBarcodeNames,stretchedConsensusBarcodes,stretchedConsensusBitmasks,eteSettings);
           
    import CBT.ExpComparison.UI.launch_export_ui;
    timestamp = datestr(clock(), 'yyyy-mm-dd_HH_MM_SS');
    sessionFilename = strcat(['ETE session ' timestamp]);
    launch_export_ui(ts, {sessionFilename}, eteSessionStruct)
    
    %TODO: allow multiple comparisons at the same time, so that 

    %TODO: cleanup, use CBT.ExpComparison.UI.generate_ete_ui instead
    ...
    set(hBtnSaveBarcodeTxt, 'Callback',  @(~, ~) ...
        feval(@(barcodeIdxA, barcodeIdxB) ...
            export_barcode_comparison_txt_wrapper( ...
                    barcodeIdxA, ...
                    barcodeIdxB, ...
                    stretchedConsensusBarcodes, ...
                    consensusBarcodeNames, ...
